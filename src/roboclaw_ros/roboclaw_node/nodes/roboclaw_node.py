...
class EncoderOdom:
    ...
    def update_publish(self, enc_left, enc_right):
        ...
        if enc_left!=None and enc_right!=None:
		if abs(enc_left - self.last_enc_left) > 20000:
        		rospy.logerr("Ignoring left encoder jump: cur %d, last %d" % (enc_left, self.last_enc_left))
            		...
            		rospy.logerr("Ignoring right encoder jump: cur %d, last %d" % (enc_right, self.last_enc_right))
                   	 #rospy.loginfo("KREMCYC: Encoders none error")
        	else:
 ...
 #       anlge_imu = data.data
	...
	r_time = rospy.Rate(10) 
        while not rospy.is_shutdown():
            ...
            if (rospy.get_rostime() - self.last_set_speed_time).to_sec() > 3:
            ...
            try:
                ...
                rospy.loginfo("KREMCYC: OSError11 M1")
    ...
    def cmd_vel_callback(self, twist):
        ...
        vr_ticks = int(vr * self.TICKS_PER_METER*1.4) # ticks/s
        vl_ticks = int(vl * self.TICKS_PER_METER*1.4)

        rospy.logdebug("vr_ticks:%d vl_ticks: %d", vr_ticks, vl_ticks)
        print("vr_ticks:%d vl_ticks: %d", vr, vl_ticks)
        try:
...
#                roboclaw.ForwardM2(self.address, 0)
#            else:
             rospy.loginfo("KREMCYC: Try to send vel")
